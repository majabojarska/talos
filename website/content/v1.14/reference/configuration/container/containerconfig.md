...
|`user` |string |Overrides the image's USER, in `uid` or `uid:gid` form.<br><br>There are no user namespaces, so a container running as uid 0 is root on the host. <details><summary>Show example(s)</summary>{{< highlight yaml >}}
user: 65534:65534
{{< /highlight >}}</details> | |
...
|`options` |[]string |Mount options. User volume mounts are read-only by default (`ro`).  |`ro`<br />`rw`<br />`noexec`<br />`nosuid`<br />`nodev`<br />`noatime`<br /> |
