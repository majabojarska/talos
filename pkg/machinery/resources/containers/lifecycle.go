...
//
// The resource carries no data: the finalizer set is the payload. Controllers which own something
// that must be released before containerd goes away put a finalizer on it, and the stopContainers
// sequencer phase tears it down and waits for those finalizers to clear.
//
// This is needed because controllers outlive services. Within the shutdown sequence, stopServices
// stops the CRI containerd together with everything depending on it, so by the time the
// volumeFinalize phase offers its own five-minute window, containerd is already gone. Context
// cancellation is later still. Hence a barrier of our own, torn down before stopServices.
//
// Modeled on block.VolumeLifecycle and k8s.KubeletLifecycle, which solve the same problem for
// volumes and the kubelet.
