...
type ContainerConfigV1Alpha1 struct {
	...
	//     normalized, so `nginx` becomes `docker.io/library/nginx:latest`.
	//   examples:
	...
	//   description: |
	//     Overrides the image's USER, in `uid` or `uid:gid` form.
	//
	//     There are no user namespaces, so a container running as uid 0 is root on the host.
	//   examples:
	//     - value: '"65534:65534"'
	ContainerUser string `yaml:"user,omitempty"`
	//   description: |
...
// User implements config.ContainerConfig interface.
func (c *ContainerConfigV1Alpha1) User() string { return c.ContainerUser }
...
// `docker.io/library/nginx:latest`. The canonical form is what the image pull uses internally and
// what image garbage collection matches against, so normalizing once here keeps the two in step.
func (c *ContainerConfigV1Alpha1) NormalizedImage() (string, error) {
	ref, err := reference.ParseDockerRef(c.ContainerImage)
	if err != nil {
...
func (c *ContainerConfigV1Alpha1) Validate(validation.RuntimeMode, ...validation.Option) ([]string, error) {
	...
	if c.ContainerImage == "" {
		...
		} else if _, digested := mustParse(normalized).(reference.Digested); !digested {
	...
	if c.ContainerUser != "" && !validUserSpec(c.ContainerUser) {
		validationErrors = errors.Join(validationErrors, fmt.Errorf("user %q must be in uid or uid:gid form", c.ContainerUser))
	}
...
func mustParse(ref string) reference.Reference {
...
func validUserSpec(user string) bool {
	...
	for _, part := range parts {
		...
		if strings.ContainsFunc(part, func(r rune) bool { return r < '0' || r > '9' }) {
			return false
		}
	}
