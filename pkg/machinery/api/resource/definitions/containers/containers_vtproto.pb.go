...
func (m *ContainerMountSpec) MarshalToSizedBufferVT(dAtA []byte) (int, error) {
	...
	if len(m.TextFilesName) > 0 {
		i -= len(m.TextFilesName)
		copy(dAtA[i:], m.TextFilesName)
		i = protohelpers.EncodeVarint(dAtA, i, uint64(len(m.TextFilesName)))
		i--
		dAtA[i] = 0x3a
	}
	if len(m.Options) > 0 {
...
func (m *TextFilesStatusSpec) MarshalVT() (dAtA []byte, err error) {
	if m == nil {
		return nil, nil
	}
	size := m.SizeVT()
...
func (m *TextFilesStatusSpec) MarshalToVT(dAtA []byte) (int, error) {
	size := m.SizeVT()
	return m.MarshalToSizedBufferVT(dAtA[:size])
}

func (m *TextFilesStatusSpec) MarshalToSizedBufferVT(dAtA []byte) (int, error) {
	if m == nil {
		return 0, nil
	}
	i := len(dAtA)
	...
	if len(m.DocumentName) > 0 {
		...
		i = protohelpers.EncodeVarint(dAtA, i, uint64(len(m.DocumentName)))
		i--
		dAtA[i] = 0x12
	}
	if len(m.ContainerId) > 0 {
...
func (m *TextFilesStatusSpec) SizeVT() (n int) {
	if m == nil {
		return 0
	}
	var l int
...
func (m *ContainerMountSpec) UnmarshalVT(dAtA []byte) error {
	...
	for iNdEx < l {
		...
		case 7:
			if wireType != 2 {
				return fmt.Errorf("proto: wrong wireType = %d for field TextFilesName", wireType)
			}
			var stringLen uint64
			for shift := uint(0); ; shift += 7 {
				if shift >= 64 {
			...
			m.TextFilesName = string(dAtA[iNdEx:postIndex])
			iNdEx = postIndex
		default:
			iNdEx = preIndex
			skippy, err := protohelpers.Skip(dAtA[iNdEx:])
...
func (m *TextFilesStatusSpec) UnmarshalVT(dAtA []byte) error {
	l := len(dAtA)
	iNdEx := 0
	for iNdEx < l {
		preIndex := iNdEx
		...
		wireType := int(wire & 0x7)
		if wireType == 4 {
			return fmt.Errorf("proto: TextFilesStatusSpec: wiretype end group for non-group")
		}
		if fieldNum <= 0 {
			return fmt.Errorf("proto: TextFilesStatusSpec: illegal tag %d (wire type %d)", fieldNum, wire)
		}
		switch fieldNum {
		case 1:
			if wireType != 2 {
