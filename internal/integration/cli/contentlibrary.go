...
// defaultLibrary is the content library hack/test/patches/content-library.yaml declares, which the
// content library CI job applies. Clusters created without that patch have no library at all.
const defaultLibrary = "default"

// ContentLibrarySuite verifies the talosctl hv content-library commands.
...
func (suite *ContentLibrarySuite) TestListUnknownLibrary() {
	...
	suite.RunCLI(
		[]string{"hv", "content-library", "list", "--nodes", node, "not-a-library"},
		base.ShouldFail(),
		base.StderrNotEmpty(),
		base.StdoutEmpty(),
	)
...
func (suite *ContentLibrarySuite) TestDeleteUnknownLibrary() {
	...
	suite.RunCLI(
		[]string{"hv", "content-library", "delete", "--nodes", node, "not-a-library", "image.raw"},
		base.ShouldFail(),
		base.StderrNotEmpty(),
		base.StdoutEmpty(),
	)
...
func (suite *ContentLibrarySuite) TestUploadRequiresSingleNode() {
	...
	suite.RunCLI(
		[]string{"hv", "content-library", "upload", "--nodes", nodes[0], "--nodes", nodes[1], "not-a-library", "/etc/hosts"},
		base.ShouldFail(),
...
func (suite *ContentLibrarySuite) TestRoundTrip() {
	...
	if !suite.libraryConfigured(node) {
		suite.T().Skipf("skipping test, node %s has no content library %q configured", node, defaultLibrary)
	}
	...
	suite.RunCLI(
		[]string{"hv", "content-library", "upload", "--nodes", node, "--name", name, "--overwrite", defaultLibrary, "-"},
		base.WithStdin(strings.NewReader("talos")),
		base.StdoutShouldMatch(nameRegexp),
	)

	suite.RunCLI(
		[]string{"hv", "content-library", "list", "--nodes", node, defaultLibrary},
		base.StdoutShouldMatch(nameRegexp),
	)

	suite.RunCLI(
		[]string{"hv", "content-library", "delete", "--nodes", node, defaultLibrary, name},
		base.StdoutEmpty(),
	)
...
func (suite *ContentLibrarySuite) libraryConfigured(node string) bool {
	return suite.MakeCMDFn([]string{"get", "--nodes", node, "contentlibrarystatus", defaultLibrary})().Run() == nil
}
