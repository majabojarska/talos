...
func (ctrl *MountController) reconcileContainer(
	...
	for _, containerMountSpec := range containerSpec.TypedSpec().Mounts {
		...
		case containers.MountKindTextFiles:
			...
			status, getErr := safe.ReaderGetByID[*containers.TextFilesStatus](ctx, runtime, statusID)
			if getErr != nil {
			...
			if status.TypedSpec().Error != "" {
				...
				reasons = append(reasons, fmt.Sprintf("text files %q: %s", containerMountSpec.TextFilesName, status.TypedSpec().Error))
