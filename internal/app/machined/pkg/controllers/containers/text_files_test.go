...
// createSpec creates a ContainerSpec declaring a textFiles mount of textFilesDocument.
func (suite *TextFilesSuite) createSpec(containerID string) {
	...
	spec.TypedSpec().Mounts = []containers.ContainerMountSpec{
		{
			...
			TextFilesName: textFilesDocument,
...
// treePath is the host directory a container's textFilesDocument tree materializes into.
func (suite *TextFilesSuite) treePath(containerID string) string {
	return filepath.Join(suite.baseDir, containerID, textFilesDocument)
}
...
func (suite *TextFilesSuite) TestMaterializesNestedTree() {
	...
	suite.createSpec(textFilesContainer)

	root := suite.treePath(textFilesContainer)
...
func (suite *TextFilesSuite) TestPerContainerCopies() {
	...
	for _, containerID := range []string{"first", "second"} {
		root := suite.treePath(containerID)
