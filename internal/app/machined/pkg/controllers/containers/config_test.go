...
func (suite *ConfigSuite) TestNormalizesImageAndResolvesDefaults() {
	...
	ctest.AssertResource(suite, "nginx", func(spec *containers.ContainerSpec, asrt *assert.Assertions) {
		// A short reference is expanded here, once, so that everything downstream (including image
		// garbage collection) sees the same canonical string the pull records.
		asrt.Equal("docker.io/library/nginx:latest", spec.TypedSpec().Image)
