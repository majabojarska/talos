...
// instance.
func (suite *StatusSuite) setInstanceStatus(mutate func(*containers.ContainerInstanceStatusSpec)) {
	status := containers.NewContainerInstanceStatus(containers.NamespaceName, containers.InstanceID(statusTestContainer, 0))
	status.TypedSpec().ContainerID = statusTestContainer
...
func (suite *StatusSuite) TestLastExitSurvivesInstanceGap() {
	...
	suite.Destroy(containers.NewContainerInstanceStatus(containers.NamespaceName, containers.InstanceID(statusTestContainer, 0)))
