...
func (ctrl *StatusController) reconcileContainer(
	...
	stopping, err := ctrl.instanceStopping(ctx, r, containerID, instance)
	if err != nil {
		return optional.None[time.Duration](), err
	}
	...
	if err := safe.WriterModify(ctx, r,
		...
		func(res *containers.ContainerStatus) error {
			...
			project(res.TypedSpec(), spec, imageStatus, instance, waitingFor, stopping, before)
...
// instanceStopping reports whether containerID's current instance is being torn down.
//
// ContainerInstanceStatus.Phase only flips to Terminated once the task has actually exited, so a
// teardown already in progress — SIGTERM sent, mounts being released — is only visible on the
// ContainerInstanceSpec's own resource phase, not on anything in its status.
func (ctrl *StatusController) instanceStopping(
	ctx context.Context,
	r controller.Reader,
	containerID string,
	instance *containers.ContainerInstanceStatus,
	...
	if err != nil {
		...
		return false, fmt.Errorf("failed to get instance spec %q: %w", instanceSpecID, err)
	}

	return instanceSpec.Metadata().Phase() == resource.PhaseTearingDown, nil
}
...
func project(
	...
	status.Image = spec.TypedSpec().Image.Ref
